...
News adapters ─▶ dedupe ─▶ cluster ─▶ junk-filter + relevance/source-tier rank
            ─▶ LLM call 1: interpret (grounded) + Mizuho TAGGING (riskId/scenarioId/confidence; may be empty) ─▶ validate ids
            ─▶ resolve tags → alignment (why = curated scenario path; no model prose)
            ─▶ LLM call 2: layman translation (grounded twin)
...
**Existing.** Provider-agnostic LLM interpretation: **Gemini-first** (`gemini-2.5-flash`, free), **Anthropic backup** (Haiku), else curated. The generation run makes **two** grounded LLM calls: (1) interpret news clusters → themes + editorial + (conditional) Japan narrative, **and tag each theme with 0–2 Mizuho Top-Risk scenarios** `{riskId, scenarioId, confidence}`; (2) **translate** every prose field into a plain-English twin. The Mizuho "why" is NOT model-written — it is the **curated scenario `path`** (executive) / `pathLayman` (plain-English), resolved locally from the validated tag. This folds the old separate alignment call into call 1 as *simple tagging*: fewer calls, no hallucinated mapping prose, and nothing extra to translate. **Gemini "thinking" is disabled (`thinkingBudget: 0`)** — as a reasoning model its hidden thinking tokens were consuming `maxOutputTokens` and truncating the JSON (`finishReason=MAX_TOKENS`), which forced slow Anthropic fallbacks that overran the 180s cron. Each step validated and isolated so its failure never breaks the briefing.
