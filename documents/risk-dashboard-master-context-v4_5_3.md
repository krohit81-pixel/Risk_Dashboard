# Risk Dashboard — Master Context (v4.5.3)
...
- **4.4** — **Screenshot input + personalized focus.** Gemini moved to **Tier 1** (10,000 RPD / 1,000 RPM / 1M TPM; `RESEARCH_DAILY_CAP`=10) — the reservation cap and provider split are now hygiene/choice, not scarcity necessities. (1) **Research image input:** a third input tier alongside text/URL. Screenshots are transcribed verbatim by Gemini multimodal (`extractFromImage`, temp 0), then the transcript runs through the **unchanged** `analyzeContent()` (two-step, auditable — the transcript is shown in a collapsible block). Up to 4 images (long articles span screenshots). Gemini-only extraction (no Anthropic image fallback in v1). (2) **“What should I focus on?”:** a dedicated, personalized call (`generateFocus`) run AFTER alignment, fed a curated role/priority profile (`FOCUS_PROFILE`) and the alignment result so it does NOT restate “Why Mizuho cares”. Deliberately **allowed to be empty** (no filler), 0–5 typed items (attention / conversation / learning), Research-only, persisted to saved analyses. A single image analysis now makes ~4 Gemini calls (extract + interpret + align + focus) — immaterial at Tier 1. **Data posture:** images go to the LLM provider via the user's own key; internal/confidential material should not be uploaded (implicit; no in-UI warning by request).
- **Post-4.4 reliability fixes.** (a) **Editorial escalation:** when Gemini returns valid JSON but too few themes (`themes.length < 3`, logged as `invalid_json`), the retry now **escalates to Anthropic** via `forceProvider` instead of re-trying Gemini, before falling back to last-good. (b) **Non-array hardening:** the model occasionally returns a theme list field (`questions`/`lenses`/`signals`) as a non-array; this crashed "Go deeper" and skipped the layman step. Fixed in three layers — theme assembly defaults missing/non-array fields to `[]`/`""`; `translateLayman` coerces via `asArray()`; the intel UI list components guard with `Array.isArray(x) && x.length` (a bare `?.length` let a string through to `.map`).
- **4.5 / 5.0 — Bloomberg ingestion (in-repo).** A Python extractor (`api/cron-bloomberg.py`) runs as a Vercel Function **inside the same project** (root `/api` + `requirements.txt`), reads Bloomberg newsletter email via IMAP, scrubs HTML, asks Gemini to **extract** (not analyze) a structured digest, and writes it to the **shared KV** the dashboard already uses. The Research tab shows the digest as a reviewable queue; each story has one-tap "Analyze this" through the existing `analyzeContent()` (Mizuho spine + focus + cap reused). Dumb pipe, smart endpoint — no second taxonomy, no second analyzer.
- **4.5.1 — TTL + staleness guard.** The digest key is written with a 36h TTL so a stalled extractor never pins a stale digest; the panel hides/mutes a digest older than ~2 days.
- **4.5.2 — Per-briefing grouping, twice-daily, run history, analyze-persistence.** Each briefing stores under its own key `bloomberg:type:{key}` with its own 36h TTL (morning/evening coexist; same briefing next day overwrites only its own key). Cron runs **twice daily** (05:30 & 19:00 IST). Each run logs `{run_time, emails_found, processed, failed, newsletter_types[]}` to `bloomberg:runs`, surfaced as a separate "Bloomberg ingestion" sub-list in the Today-tab history. Analyzed stories are recorded in `bloomberg:analyzed` (KV, 48h TTL) so the panel's ✓ persists across reloads (previously in-memory only — a bug).
- **4.5.3 — Detection fix.** Briefing classification is deterministic from the **footer subscription line** ("subscribed to Bloomberg's {name} newsletter") first, then subject + masthead image alt, then body — fixing creative-subject emails (e.g. subject "AI freakout" for an Evening Briefing) that previously fell into a generic bucket because the masthead is an image and the subject doesn't name the briefing.
...
> **V4.5 addition — Bloomberg ingestion (in-repo, polyglot).** Alongside the Next.js app, a Python Vercel Function (`api/cron-bloomberg.py`, root `/api` + `requirements.txt`) ingests Bloomberg newsletter email and writes per-briefing digests + run records to the **same Vercel KV** the dashboard reads. The dashboard never calls the extractor; they communicate only through KV keys (`bloomberg:type:{key}`, `bloomberg:analyzed`, `bloomberg:runs`). This keeps the extractor a replaceable “dumb pipe” and the dashboard the sole interpreter. Mixed Node+Python in one Vercel project is supported; if it ever causes build friction, the extractor can split into its own project using the same KV vars (clean fallback).
...
- **Gemini Tier 1 (v4.4).** 10,000 RPD / 1,000 RPM / 1M TPM. The 20-RPD scarcity that drove the reservation cap, provider split, and transient-retry is gone — these remain as good hygiene, not necessity. Research image analysis costs ~4 Gemini calls; still immaterial.
- **Bloomberg email scraping (v4.5).** Ingestion depends on Bloomberg's newsletter HTML; `clean_html` may need occasional adjustment if they restructure it. Briefing detection relies primarily on the footer “subscribed to Bloomberg's {name} newsletter” line plus a curated phrase vocabulary (`NEWSLETTER_TYPES`); a renamed briefing needs a new phrase added. Unmatched mail lands in a generic `bloomberg_other` bucket (not lost).
- **Mixed-runtime repo (v4.5).** Node + Python in one Vercel project is supported but less common; the extractor can be split to its own project (same KV) if the Python build ever causes friction.
- **Image data posture (v4.4).** Screenshots are sent to the LLM provider (Gemini) via the user's own API key for transcription; ephemeral unless the analysis is saved. Internal/confidential material should not be uploaded — treated as implicit, no in-UI guard by user preference.
...
**Completed in 4.3:** Transient Gemini retry; robust theme persistence; “what’s new”; risk↔implication link. Cosmetic: saved-card colours to defined tokens.

**Completed in 4.4:** Research screenshot/image input; personalized “What should I focus on?”. Gemini Tier 1.

**Completed post-4.4:** editorial Anthropic-escalation on under-production; non-array crash hardening (Go deeper / layman).

**Completed in 4.5.x:** in-repo Bloomberg extractor (`api/cron-bloomberg.py`) → shared KV → Research queue; per-briefing grouping + own TTLs; twice-daily cron; Bloomberg run history on Today; analyze-✓ persistence; footer-based briefing detection.
...
**As of v4.5.3:** live on Vercel Pro. Four tabs (Today · Markets · Research · Learn) plus an in-repo Python Bloomberg ingestion function. Gemini Tier 1 primary; Anthropic fallback/weekly/escalation. KV is the single store shared by the dashboard and the extractor.
